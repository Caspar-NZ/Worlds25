...
@TeleOp (name="iTeleOp", group="Worlds")
public class iteleOp extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            ...
            if (currentGamepad2.dpad_up && !previousGamepad2.dpad_up && horizontalSlides.getCurrentPosition() < slideLimit){
                intake.setInnerBlockOpen(true);
                intake.setOuterBlockOpen(true);
                intake.setRotation(TRANSFER);
            }
            if (currentGamepad2.dpad_down && !previousGamepad2.dpad_down && horizontalSlides.getCurrentPosition() < slideLimit){
                intake.setInnerBlockOpen(true);
                intake.setOuterBlockOpen(true);
                intake.setRotation(TUCKED);
            }
            ...
            if (!goingHome && !transferringYellow) {
                ...
                if (horiSlidesTarget > currentHoriPos){
                    if (horiSlidesTarget >= slideLimit){
                        horiSlidesTarget = slideLimit;
                    }
                }
            }
